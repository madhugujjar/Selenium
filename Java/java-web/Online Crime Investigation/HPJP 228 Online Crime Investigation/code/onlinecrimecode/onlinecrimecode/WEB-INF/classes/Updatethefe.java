...
import java.io.*;
import CMST.*;
public class Updatethefe extends HttpServlet
{
  ...
  }else
  {
   ...
   pstmt=con.prepareStatement("update thieves  set th_name=?,location=?,no_of_years=?,type_th=?,photo=? where thieve_id='"+ tid + "'");
    ...
    if(rows==1)
    {
	 ...
	 out.println("<html><script>function check(){self.close();opener.location='ThievesInformation.jsp'; }</script><center><h1><b>Details Updated Successfully</b></h1></center></html>");
